...
from model.SEAttention import *
BN_MOMENTUM = 0.1
from model.dyrelu import *
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.dyrelu = DyReLUB(channels = planes, conv_type = '2d')
        # self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        # self.Attention = SEAttention(channel=planes,reduction=8)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn1 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
        self.dyrelu1 = DyReLUB(channels = planes, conv_type = '2d')
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
        self.dyrelu2 = DyReLUB(channels = planes, conv_type = '2d')
        self.conv3 = nn.Conv2d(planes, planes * self.expansion, kernel_size=1,
                               bias=False)
        self.bn3 = nn.BatchNorm2d(planes * self.expansion,
                                  momentum=BN_MOMENTUM)
        self.dyrelu3 = DyReLUB(channels = planes * self.expansion, conv_type = '2d')
        # self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
    ...
    def __init__(self, input_branches, output_branches, c):
        ...
        for i in range(self.input_branches):  # 每个分支上都先通过4个BasicBlock
            ...
            branch = nn.Sequential(
                ...
                BasicBlock(w, w),
                BasicBlock(w, w),
                SEAttention(channel=w, reduction=16)
            )
            # branch[4].init_weights()
        ...
        for i in range(self.output_branches):
                    ...
                    for k in range(i - j - 1):
                        ops.append(
                            nn.Sequential(
                                ...
                                DyReLUB(channels = c * (2 ** j), conv_type = '2d')
            ...
            self.dyrelu.append(DyReLUB(channels = c * (2 ** i), conv_type = '2d'))
        # self.relu = nn.ReLU(inplace=True)
...
class HighResolutionNet(nn.Module):
    def __init__(self, base_channel: int = 32, num_joints: int = 17):
        ...
        # self.relu = nn.ReLU(inplace=True)
        self.dyrelu = DyReLUB(channels = 64, conv_type = '2d')
        ...
        self.transition1 = nn.ModuleList([
            nn.Sequential(
                ...
                # nn.ReLU(inplace=True)
                DyReLUB(channels = base_channel, conv_type = '2d')
            ),
            nn.Sequential(
                nn.Sequential(  # 这里又使用一次Sequential是为了适配原项目中提供的权重
                    ...
                    # nn.ReLU(inplace=True)
                    DyReLUB(channels = base_channel * 2, conv_type = '2d')
                )
            )
        ])
        ...
        self.transition2 = nn.ModuleList([
            ...
            nn.Sequential(
                nn.Sequential(
                    ...
                    # nn.ReLU(inplace=True)
                    DyReLUB(channels = base_channel *4, conv_type = '2d')
                )
            )
        ])
        ...
        self.transition3 = nn.ModuleList([
            ...
            nn.Sequential(
                nn.Sequential(
                    ...
                    # nn.ReLU(inplace=True)
                    DyReLUB(channels = base_channel * 8, conv_type = '2d')
                )
            )
        ])
